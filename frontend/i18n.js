...
const STRINGS = {
  en: {
    ...
    "server.hintKeyless": "Passwordless (key/agent) login is required — leave the key file blank to use <code>~/.ssh/id_*</code> / ssh-agent. The connection and tmux availability are checked before saving; remote terminals automatically create or reconnect persistent tmux sessions.",
  ...
  zh: {
    ...
    "server.hintKeyless": "必须使用免密（密钥/agent）登录 — 密钥文件留空即使用 <code>~/.ssh/id_*</code> 或 ssh-agent。保存前会检测连接和 tmux；远程终端会自动创建或重连持久化 tmux 会话。",
